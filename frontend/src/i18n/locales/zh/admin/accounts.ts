export default {
    accounts: {
      ...
      a2gImportHint: '填写 Grok2API 地址与 Admin Key，由 Sub2 服务端直连接入 G2A 号池并导入（SSO→OAuth）。也可展开高级选项使用文件/粘贴。',
      a2gImportWarning: '重复 SSO 与重复 Email 一律跳过，不会覆盖已有账号。导入成功会写入 credentials.sso，便于与 G2A 双向去重。',
      a2gDirectTitle: '网站直连 Grok2API',
      a2gBaseUrl: 'Grok2API 地址',
      a2gBaseUrlPlaceholder: 'http://127.0.0.1:8010',
      a2gAdminKey: 'Grok2API Admin Key',
      a2gAdminKeyPlaceholder: '与 G2A 管理后台登录密钥相同',
      a2gFetchButton: '从 G2A 拉取号池',
      a2gFetching: '正在拉取 G2A 号池…',
      a2gFetchedCount: '已拉取 {count} 个 SSO，可点下方开始导入',
      a2gDirectNote: '由 Sub2 后端服务端访问 G2A（避免浏览器 CORS / Failed to fetch）。本机可填 http://127.0.0.1:8010；若失败自动尝试 host.docker.internal / 172.24.80.1。Admin Key 必须是 G2A app_key。',
      a2gMissingFields: '请填写 Grok2API 地址和 Admin Key',
      a2gFetchFailed: '从 G2A 拉取号池失败',
      a2gFetchFailedDetail: 'G2A 返回 HTTP {status}: {detail}',
      a2gFetchEmpty: 'G2A 号池为空或响应中没有 token',
            a2gFetchTimeout: '拉取 G2A 超时（20s），请确认 G2A 已启动',
      a2gKeyHint: 'Admin Key 必须是 G2A 的 app_key（管理后台登录密钥），不是 api_key。Docker 场景可用 http://172.24.80.1:8010',
      a2gFetchCorsHint: '请确认 G2A 已启动；现已改为 Sub2 服务端拉取，若仍失败请检查 Docker 能否访问 G2A 地址（可用 http://172.24.80.1:8010）',
      a2gAdvanced: '高级：文件 / 粘贴导入',
      a2gImportFile: 'G2A 导出文件',
      a2gImportPaste: '或粘贴导出内容',
      a2gImportPastePlaceholder: '每行一个 SSO，或粘贴 Grok2API JSON 导出',
      a2gImportSelectFile: '请先直连拉取 G2A 号池，或选择文件/粘贴内容',
