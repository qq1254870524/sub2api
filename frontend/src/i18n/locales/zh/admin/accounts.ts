export default {
    accounts: {
      ...
      a2gImportHint: '直接填写 Grok2API 地址与 Admin Key，浏览器会拉取对端号池 SSO 并导入到 Sub2API（SSO→OAuth）。也可展开高级选项使用文件/粘贴。',
      ...
      a2gDirectNote: '由当前浏览器访问 G2A（不经 Sub2 容器出网）。G2A 需本机可打开，例如 http://127.0.0.1:8010',
      ...
      a2gKeyHint: 'Admin Key 必须是 G2A 的 app_key（管理后台登录密钥），不是 api_key。旧版若卡在 8010 可改用 http://127.0.0.1:8012',
      a2gFetchCorsHint: '请确认 G2A 已启动且浏览器能打开该地址；跨域需 G2A 允许 CORS',
