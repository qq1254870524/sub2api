export default {
    accounts: {
      ...
      a2gImportHint: 'Enter the Grok2API base URL and Admin Key. Your browser pulls the remote pool SSO tokens and imports them into Sub2API (SSO→OAuth). File/paste remains available under Advanced.',
      a2gImportWarning: 'Duplicate SSO and duplicate email are always skipped and never overwrite existing accounts. Successful imports store credentials.sso for G2A cross-pool dedupe.',
      a2gDirectTitle: 'Direct connect to Grok2API',
      a2gBaseUrl: 'Grok2API base URL',
      a2gBaseUrlPlaceholder: 'http://127.0.0.1:8010',
      a2gAdminKey: 'Grok2API Admin Key',
      a2gAdminKeyPlaceholder: 'Same key used to log into the G2A admin UI',
      a2gFetchButton: 'Fetch pool from G2A',
      a2gFetching: 'Fetching G2A pool…',
      a2gFetchedCount: 'Fetched {count} SSO tokens — click import below',
      a2gDirectNote: 'Fetched by this browser (not via Sub2 container egress). G2A must be reachable locally, e.g. http://127.0.0.1:8010',
      a2gMissingFields: 'Please fill Grok2API base URL and Admin Key',
      a2gFetchFailed: 'Failed to fetch G2A pool',
      a2gFetchFailedDetail: 'G2A returned HTTP {status}: {detail}',
      a2gFetchEmpty: 'G2A pool is empty or response has no tokens',
            a2gFetchTimeout: 'Fetch from G2A timed out (20s); ensure G2A is running',
      a2gKeyHint: 'Admin Key must be G2A app_key (admin UI login key), not api_key. If port 8010 is stale, try http://127.0.0.1:8012',
      a2gFetchCorsHint: 'Ensure G2A is running and reachable in the browser; CORS must allow this origin',
      a2gAdvanced: 'Advanced: file / paste import',
      ...
      a2gImportSelectFile: 'Fetch from G2A first, or select a file / paste content',
