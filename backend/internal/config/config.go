...
type GatewayConfig struct {
	...
	// <=0 表示不限制，失败时会继续切换直到号池无可选账号（适合 800+ 大号池）。
	MaxAccountSwitches int `mapstructure:"max_account_switches"`
	// Gemini 账户切换最大次数（Gemini/Antigravity 平台单独配置）。
	// <=0 表示不限制，语义同上。
...
func setDefaults() {
	...
	viper.SetDefault("gateway.max_account_switches", 0)          // 0=全池切换（不人为截断）
	viper.SetDefault("gateway.max_account_switches_gemini", 0) // 0=全池切换
